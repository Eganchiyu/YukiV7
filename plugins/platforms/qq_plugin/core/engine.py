...
from yuki_core.identity import get_base_setting, get_summary_prompt
from yuki_core.config import cfg
from yuki_core.identity import build_ice_break_prompt
from plugins.platforms.qq_plugin.core.prompts import build_chat_context
from yuki_core.identity import get_base_setting, get_summary_prompt
from yuki_core.config import cfg
from yuki_core.identity import build_ice_break_prompt
from plugins.platforms.qq_plugin.core.prompts import build_chat_context
...
class YukiEngine:
    def __init__(self, rag, history_manager, yuki_state, sender):
        ...
        self.llm_caller = yuki_chat  # V7 LLM 调用器
    ...
    async def api_reply(self, chat_id: str, combined_text: str, history_dict: dict, mode,
        ...
        try:
            Yuki_Answer = await self.llm_caller(
    ...
    async def do_summarize(self, chat_id, history):
        ...
        try:
            diary_content = await self.llm_caller(
                messages=[
