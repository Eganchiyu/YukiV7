...
from plugins.platforms.qq_plugin.modules.vision.utils import log
from plugins.platforms.qq_plugin.modules.vision.cache import MemeCache
import logging
...
class MemeProcessor:
    def __init__(self):
        from plugins.platforms.qq_plugin.llm_call import yuki_vision_chat
        self.cache = MemeCache()
        self.semaphore = asyncio.Semaphore(cfg.MAX_CONCURRENT_MEME)
        self._vision_chat = yuki_vision_chat
    ...
    async def call_api(self, b64_data):
        ...
        try:
            ...
            logger.warning(f"Vision provider 调用失败，降级到 HTTP: {e}")
